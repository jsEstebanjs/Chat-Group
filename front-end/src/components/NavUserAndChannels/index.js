...
import { MdAdd, MdKeyboardArrowDown, MdAccountCircle, MdLogout, MdClose } from "react-icons/md";
...
function NavUserAndChannels({ img, funHandle, visible }) {
    ...
    return (
        <>
        <div onClick={()=> funHandle()} className={`${styles.opacity} ${visible ? styles.opacityVisible : null}`}></div>
            <div className={`${styles.mainContainerNavUserAndChannels} ${visible ? styles.mainContainerNavUserAndChannelsVisible : null}`}>
                ...
                <div onClick={() => funHandle()} className={styles.containerClose}>
                    <MdClose />
                </div>
